...
int main(){
    ...
    struct sembuf *end=(struct sembuf*) calloc(1, sizeof(struct sembuf));
    
    setbuffers(&init[0], FREEAR, 0, 0);
    setbuffers(&init[1], FREEAR, 1, 0);
    setbuffers(&init[2], SENDIX, 1, 0);
    setbuffers(&init[3], SENDNR, -1, 0);
    setbuffers(&end[0], FREEAR, -1, 0);

    int ix;
    while(1){
        if(semctl(sem_id, SENDNR, GETVAL, NULL) > 0){
            semop(sem_id, init, 4);

            ix = semctl(sem_id, SENDIX, GETVAL, NULL);
            ix = (ix - 1 + PROD) % PROD;
