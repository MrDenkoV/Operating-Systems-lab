...
int createfiles(FILE* list, struct operation **ptr){
    ...
    while ((read = getline(&line, &len, list)) != -1) {
        ...
        ops[ob].B.col = width;
        ops[ob].B.row = (f_size / width / 5);


        FILE* fileC = fopen(fileCName, "w");
...
void manager(char* name, int n, int s, int type){
    ...
    if(ix!=-1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if(s < (double)(end - begin) / CLOCKS_PER_SEC)
                exit(i);
        ...
        int tmp=0;
        for(int i=0; i<n; i++){
            waitpid(child_pid[i], &tmp, 0);
            printf("Proces %d wykonał %d mnożeń macierzy\n", (int)child_pid[i], WEXITSTATUS(tmp));
        }
    }
