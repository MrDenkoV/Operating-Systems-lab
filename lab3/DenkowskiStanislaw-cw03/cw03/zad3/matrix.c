...
void manager(char* name, int n, int s, int type, int mem, int tim){
    ...
    struct rlimit tilim;
    tilim.rlim_cur=tim+1;
    tilim.rlim_max=tim;
    struct rlimit melim;
    melim.rlim_cur=mem*1024*1024+1;
    melim.rlim_max=mem*1024*1024;
    ...
    for(int i=0; i<n; i++){
        ...
        if(child_pid[i]==0) {
            ix = i;
            setrlimit(RLIMIT_CPU, &tilim);
            setrlimit(RLIMIT_AS, &melim);
            break;
        }
    }
    ...
    if(ix!=-1) {
        ...
        getrusage(RUSAGE_CHILDREN, &prev);
        for(int i=0; i<n; i++){
            ...
            printf("Times:\n");
            printf("Sys: %d\n", abs(used.ru_stime.tv_usec-prev.ru_stime.tv_usec));
            printf("User: %d\n\n", abs(used.ru_utime.tv_usec-prev.ru_utime.tv_usec));
            prev=used;
        }
    }
...
int main(int argc, char *argv[]){
    ...
    manager(argv[1], atoi(argv[2]), atoi(argv[3]), strcmp(argv[4], "one"), atoi(argv[5]), atoi(argv[6]));
    return 0;
}
